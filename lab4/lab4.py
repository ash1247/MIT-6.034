...
def forward_checking_prop_singleton(state, verbose=False):
    ...
    queue = deque([v for v in state.get_all_variables() if v.domain_size() == 1])
    visited = []
    while queue:
        X = queue.pop()
        visited.append(X)
        ...
        for c in C:
            ...
            for y in Y.get_domain():
                ...
                if Y.domain_size() == 0:
                    return False
        queue.extend([v for v in state.get_all_variables() if (v.domain_size() == 1 and v not in queue and v not in visited )])
    return True
...
def information_disorder(yes, no):
    if yes or no:
        return sum([len(b) * sum([class_disorder(b, c) for c in set(b)] )
                    for b in [yes, no] ]) / ( len(yes) + len(no) )
    else: 
        return 0
...
WHAT_I_FOUND_INTERESTING = "aa"
WHAT_I_FOUND_BORING = "bb"
